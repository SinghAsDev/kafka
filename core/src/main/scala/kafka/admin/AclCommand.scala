...
object AclCommand {
  ...
  def main(args: Array[String]) {
    ...
    if (opts.options.has(opts.helpOpt))
      CommandLineUtils.printUsageAndDie(opts.parser, description(opts))
  ...
  private def description(opts: AclCommandOptions): String = {
    withAuthorizer(opts) { authorizer => return authorizer.description()}
    ""
  }
  ...
  private def listAcl(opts: AclCommandOptions) {
    withAuthorizer(opts) { authorizer =>
      ...
      val resourceToAcls: Iterable[(Resource, java.util.Set[Acl])] =
        if (resources.isEmpty) authorizer.getAcls().asScala
  ...
  private def validateOperation(opts: AclCommandOptions, resourceToAcls: Map[Resource, Set[Acl]]) = {
    for ((resource, acls) <- resourceToAcls) {
      ...
      if ((acls.map(_.getOperation) -- validOps).nonEmpty)
