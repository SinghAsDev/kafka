...
class SimpleAclAuthorizer extends Authorizer with Logging {
  ...
  override def authorize(session: Session, operation: Operation, resource: Resource): Boolean = {
    val principal = session.getPrincipal
    val host = session.getClientAddress.getHostAddress
    val acls = getAcls(resource)
    acls.addAll(getAcls(new Resource(resource.getResourceType, Resource.WILDCARD_RESOURCE)))
  ...
  private def aclMatch(session: Session, operations: Operation, resource: Resource, principal: KafkaPrincipal, host: String, permissionType: PermissionType, acls: java.util.Set[Acl]): Boolean = {
    acls.asScala.find ( acl =>
      acl.getPermissionType == permissionType
        && (acl.getPrincipal == principal || acl.getPrincipal == Acl.WILDCARD_PRINCIPAL)
        && (operations == acl.getOperation || acl.getOperation == Operation.ALL)
        && (acl.getHost == host || acl.getHost == Acl.WILDCARD_HOST)
    ).map { acl: Acl =>
  ...
  override def getAcls(resource: Resource): java.util.Set[Acl] = {
    inReadLock(lock) {
  ...
  override def getAcls(principal: KafkaPrincipal): java.util.Map[Resource, java.util.Set[Acl]] = {
    inReadLock(lock) {
      aclCache.mapValues { versionedAcls =>
        versionedAcls.acls.filter(_.getPrincipal == principal).asJava
      }.filter { case (_, acls) =>
        !acls.isEmpty
      }.toMap.asJava
    }
  ...
  override def getAcls(): java.util.Map[Resource, java.util.Set[Acl]] = {
    inReadLock(lock) {
  ...
  def toResourcePath(resource: Resource): String = {
    SimpleAclAuthorizer.AclZkPath + "/" + resource.getResourceType + "/" + resource.getName
  }
  ...
  private[auth] def aclToMap(acl: Acl): Map[String, Any] = {
    Map(PrincipalKey -> acl.getPrincipal.toString,
      PermissionTypeKey -> acl.getPermissionType.name,
      OperationKey -> acl.getOperation.name,
      HostsKey -> acl.getHost)
  }
